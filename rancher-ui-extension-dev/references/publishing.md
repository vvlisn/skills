...
> this is the default branch we use on the workflow. You can change the target branch if you need to on the workflow `build-extension-charts.yml` that is shipped with the skeleton app
...
#### Proper Tagged Release naming scheme to build Extension Helm Charts
...
If your extensions repository doesn't include a github workflow to automate the publishing procedure for a Catalog Image (ignore this step if you have it already), you can create one in `.github/workflows/build-catalog.yaml` on your extension folder, with the following content:
...
This will leverage the usage of our reusable workflow and give your Github extension repo the ability to publish to a github registry via  **Tagged Release**. 

It's mandatory that release type is **Tagged Release**, otherwise the `build-extension-catalog.yml` workflow won't run properly, which is crucial for the success of the publish operation.

When creating a Tagged Release, the tag name **_MUST_** match the proper Extension name depending on the type of build.
